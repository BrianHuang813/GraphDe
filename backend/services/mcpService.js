...
/**
 * Execute a MCP query based on extracted intent
 * @param {Object} intent - Extracted intent with parameters
 * @returns {Object} Query result
 */
export async function executeMCPQuery(intent) {
  try {
    const { category, method, parameters } = intent;

    return await send_node_Request( parameters.chain, category, method, parameters.params, process.env.NODIT_API_KEY);

    // "web3":
    //  return await send_web3_RpcRequest( parameters.chain, parameters.category, parameters.method, parameters.accountAddress, process.env.NODIT_API_KEY);
